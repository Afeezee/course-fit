...
**Railway is the recommended path** — see [`docs/DEPLOY.md`](docs/DEPLOY.md).
The API bundle (~900 MB with xgboost + scikit-learn + pandas) exceeds
Vercel's 500 MB Python serverless function limit, so the API needs a
container runtime. Railway runs it as a Docker container (persistent
process, no cold starts, connection pooling works).

Vercel remains fine for **web-only** hosting if you want split
vendors — see [`docs/VERCEL.md`](docs/VERCEL.md) for that path.
Either way, both are free-tier friendly.
